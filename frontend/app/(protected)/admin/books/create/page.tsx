...
export default function CreateBookPage() {
  ...
  const { register, handleSubmit, formState: { errors }, watch } = useForm<BookForm>();
  ...
  const onSubmit = async (data: BookForm) => {
    ...
    try {
      ...
      showToast(`"${data.title}" has been created successfully!`, 'success');
      setTimeout(() => {
        router.push('/admin/books');
      }, 1500);
    } catch (err: any) {
      const errorMessage = err.response?.data?.message || 
        (err.response?.data?.errors ? Object.values(err.response.data.errors).flat().join(', ') : 'Failed to create book');
    ...
    <Layout>
      ...
      <div className="px-4 py-6 max-w-3xl animate-fadeIn">
        ...
        <div className="mb-6">
          ...
          <p className="text-gray-500">Fill in the details to add a new book to the library</p>
        </div>
